...
export class RFQSupplierDetailComponent implements OnInit {
        ...
        this.rfqSupplierDetailList = data.data;
         if(this.rfqSupplierDetailList.projectList){
            this.rfqSupplierDetailList.projectList.forEach(element => {
              this.materialCount = this.materialCount + element.materialList.length;
              if(element.gst == null || element.gst == undefined || element.gst == ""){
                  element.gst = 'IGST';
              }
              });
        }
  ...
  submitBid(rfqSupplierObj){
    this.rfqService.postRFQDetailSupplier(this.activatedRoute.snapshot.params["supplierId"], rfqSupplierObj).then(res => {
      res.data;
      this.router.navigate(["rfq-bids/after-submit/" + this.brandCount + "/"+this.materialCount]);
    });
  }
